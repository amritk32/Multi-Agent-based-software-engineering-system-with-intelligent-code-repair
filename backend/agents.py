...
LLM1 = ChatOpenAI(model="gpt-5-mini", temperature=0, api_key=OPEN_AI_API)
...
⚠️ CRITICAL CONSTRAINT: You MUST generate code ONLY in Python. No other 
programming language is permitted. All generated code must be valid, executable 
Python code that can run on a standard Python 3.8+ interpreter.
...
the generated single-file Python application.

⚠️ CRITICAL CONSTRAINT: All test cases MUST be written in Python only. Test 
code, execution steps, and any code snippets must be valid, executable Python 
that can run on a standard Python 3.8+ interpreter. No other programming 
...
- Generated source code (which is Python)
...
"""
    ...
    def __init__(self, llm1: ChatOllama, llm2: ChatOllama) -> None:
    ...
    def code_writing_agent(
        ...
        """Generate code and forward each model token as it arrives."""
        prompt = (
            f"Requirements:\n{requirements}\n\n"
            f"Architecture:\n{architecture}\n\n"
            f"Boilerplate:\n{boilerplate}\n\n"
        ...
        return self._stream_text_llm1(
            self.CODE_WRITING_SYSTEM_PROMPT,
            prompt,
            on_token,
        )
    ...
    def generate_test_cases(
        ...
        structured_llm = self.llm1.with_structured_output(TestCaseList)

        print("🧪 Calling Qwen for structured test cases...")

        result = structured_llm.invoke(
            [
                SystemMessage(content=self.TEST_CASE_GENERATION_SYSTEM_PROMPT),
